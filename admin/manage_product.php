...
if($msg=='')
{
	...
	if(isset($_GET['id']) && $_GET['id']!='')
	{
		if($_FILES['image']['name']!='')
		{
			...
			qty='$qty',short_desc='$short_desc',description='$description',
	...
	else
	{
		...
		product(categories_id,name,mrp,price,qty,short_desc,description,meta_title,meta_desc,meta_keyword,status,image) values('$categories_id','$name','$mrp','$price','$qty','$short_desc','$description','$meta_title','$meta_desc','$meta_keyword','1','$image')");
		echo "Thank you, you form has been submitted successfully!"; 
						  ...
						  order by categories asc");
						   while($row=mysqli_fetch_assoc($res)){
							if($row['id']==$categories_id){
								echo "<option selected value=".$row['id'].">".$row['categories'].
							"</option>";
						  }else{
						   ...
						   class="form-control"><?php echo $meta_des?></textarea>
						    </div>
